...
コードレビューを依頼された場合は、使用するコーディングエージェントにかかわらず、`.agents/skills/code-review/SKILL.md`を読み、その手順に従うこと。レビューの役割定義・観点・出力形式は同ファイルに一元管理しており、サブエージェントへ委任する場合も同じスキルを適用する。レビューではコードを修正しない。
